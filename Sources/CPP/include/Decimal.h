...
//    unsigned int flags;
	...
	return strtoull(const_cast<char*>(str.c_str()), 0, 0);
...
static std::string decimalStringToDisplay(Decimal value) {
    ...
    unsigned long expPos = tempStr.find("E"); // find position of 'E' char (e.g. 2)
    if (expPos < 0) {
    ...
    unsigned long numLength = tempStr.size() - expStr.size() - 1; // length of numbers substring
    ...
    for (unsigned long i = numLength; i <= (-exp); i++) {
        result += '0';
        if (i == numLength) {
            result += '.';
            decPtAdded = true;
